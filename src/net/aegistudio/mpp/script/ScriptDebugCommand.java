...
/**
 * This debugging class in temporarily written in this way. I'm not sure whether there'll be refactors in
 * the next or more versions.
 * 
 * @author aegistudio
 */
	...
	protected void subhandle(MapPainting painting, MapCanvasRegistry canvas, CommandSender sender, String[] arguments) {
		if(arguments.length == 0) {
			...
			if(!(canvas.canvas instanceof ScriptCanvas)) {
				sender.sendMessage(notScript.replace("$canvasName", canvas.name));
				return;
			}
			...
			for(String arg : arguments) if(arg.startsWith("@")) {
				char[] carray = arg.toCharArray();
				for(int i = 1; i < carray.length; i ++) {
					if(carray[i] == 'e') eraseCassette = true;
					if(carray[i] == 'l') reloadScript = true;
					if(carray[i] == 'r') rebootScript = true;
					if(carray[i] == 'R') {
						eraseCassette = true;
						reloadScript = true;
						rebootScript = true;
